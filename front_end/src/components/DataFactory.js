export const DataFactory=(data)=>{
    ...
    // const getAlldata=Object.values(data).map((rowData,index)=>{
    //     // when reach the last 2nd element in the row
        
    //     if(data.length-2===index){
    //     const last2nd_row_values = Object.values(rowData)
    //     for(var i=0;i<last2nd_row_values.length-1;i++){
    //         if (typeof last2nd_row_values[i] == 'number'){
    //             sumLastDay_v+=last2nd_row_values[i]
    //         }
    //     }
    //     sumLastDay= sumLastDay_v.toLocaleString()
    //     }
